...
import { 
  ...
  updateDoc, deleteDoc, setDoc, getDoc, serverTimestamp,
...
// --- UTILIDADES FINANCIERAS Y SISTEMA ---
...
const Styles = memo(() => (
  <style>{`
    ...
    .hud-panel { background: var(--ff-panel); border: 1px solid rgba(255, 69, 0, 0.6); position: relative; clip-path: polygon(30px 0, 100% 0, 100% calc(100% - 30px), calc(100% - 30px) 100%, 0 100%, 0 30px); backdrop-filter: blur(20px); box-shadow: 0 0 40px rgba(0,0,0,0.9), inset 0 0 20px rgba(255,69,0,0.1); transition: all 0.4s; }
    .hud-panel::before { content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: linear-gradient(45deg, transparent 40%, rgba(255,255,255,0.05) 50%, transparent 60%); background-size: 200% 200%; animation: glass-shine 4s infinite linear; pointer-events: none; z-index: 1; }
    @keyframes glass-shine { 0% { background-position: 200% 0; } 100% { background-position: -200% 0; } }
    .hud-panel:hover { border-color: var(--ff-yellow); box-shadow: 0 0 60px rgba(255, 69, 0, 0.6), inset 0 0 30px rgba(255,215,0,0.2); transform: scale(1.02) translateY(-5px); z-index: 20; }
    ...
    .input-ff { background: rgba(0, 0, 0, 0.85); border: 2px solid #444; border-radius: 8px; color: white; font-family: 'Rajdhani', sans-serif; font-weight: 800; letter-spacing: 1px; padding-left: 3.5rem !important; transition: all 0.4s; box-shadow: inset 0 0 10px rgba(0,0,0,0.8); }
    .input-ff:focus { border-color: var(--ff-yellow); outline: none; background: rgba(255, 69, 0, 0.15); box-shadow: 0 0 25px rgba(255, 69, 0, 0.4), inset 0 0 15px rgba(255,215,0,0.2); transform: scale(1.02); }
    
    .btn-ff { background: linear-gradient(135deg, #FFD700 0%, #FF4500 50%, #8B0000 100%); color: white; font-family: 'Black Ops One', cursive; text-transform: uppercase; border: 2px solid #FFD700; clip-path: polygon(15% 0, 100% 0, 100% 65%, 85% 100%, 0 100%, 0 35%); transition: all 0.3s; position: relative; overflow: hidden; cursor: pointer; text-shadow: 2px 2px 0 rgba(0,0,0,0.8); box-shadow: 0 10px 20px rgba(0,0,0,0.5); z-index: 1; }
    .btn-ff::after { content: ''; position: absolute; top: -50%; left: -50%; width: 200%; height: 200%; background: linear-gradient(to right, rgba(255,255,255,0) 0%, rgba(255,255,255,0.3) 50%, rgba(255,255,255,0) 100%); transform: rotate(45deg); transition: all 0.3s; z-index: -1; left: -100%; }
...
const ChatSystem = ({ orderId, currentUserRole, currentUserId, orderStatus, onUpdateStatus, orderData, isMuted }) => {
  ...
  const sendMessage = async (text, type = 'text', imageUrl = null) => {
    ...
    try { 
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'orders', orderId, 'messages'), { 
        text, type, imageUrl, senderId: currentUserId || 'GUEST', role: currentUserRole, createdAt: serverTimestamp() 
      }); 
      setNewMessage(''); 
  ...
  return (
    <>
      {focusMode && <div className="fixed inset-0 bg-black/90 z-[105] pointer-events-none backdrop-blur-sm transition-all duration-500"></div>}
      <div className={`flex flex-col h-[500px] bg-black/90 rounded-lg overflow-hidden relative shadow-[0_0_40px_rgba(255,69,0,0.6)] backdrop-blur-md transition-all duration-500 ${focusMode ? 'z-[110] scale-[1.02] border-4 border-orange-500' : 'border-2 border-orange-600/50'}`}>
        ...
        <div className="bg-gradient-to-r from-gray-900 via-red-900/30 to-gray-900 p-4 border-b border-orange-600/50 flex flex-wrap justify-between items-center gap-2 relative z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            {orderData?.supportRequested && <span className="text-purple-400 font-bold text-xs flex items-center gap-1 bg-purple-900/40 px-2 py-1 rounded border border-purple-500"><Headset size={14} className="animate-pulse"/> SOPORTE ACTIVO</span>}
        ...
        <div className="flex-grow overflow-y-auto p-4 space-y-4 custom-scrollbar relative z-10">
          {orderData?.supportAgent && <div className="text-center sticky top-0 z-10"><span className="bg-purple-900/80 text-purple-300 text-xs px-4 py-1 rounded-full border border-purple-500 font-bold tracking-widest shadow-[0_0_10px_purple]">AGENTE {orderData.supportAgentName} EN LÍNEA</span></div>}
              ...
              <div key={msg.id} className={`flex flex-col max-w-[85%] animate-enter ${isSupport ? 'mx-auto' : isMe ? 'items-end ml-auto' : 'items-start mr-auto'}`}>
                {msg.type === 'system' ? (
                  ...
                  <div className={`p-4 text-sm font-bold tracking-wide relative ${isSupport ? 'chat-bubble-support text-white' : isMe ? 'chat-bubble-me text-white' : 'chat-bubble-other text-gray-100'}`}>
                    {isSupport && <div className="text-[10px] text-purple-300 font-black mb-1 flex items-center gap-1"><Shield size={12}/> AGENTE TECNOBYTE</div>}
        ...
        {orderStatus !== 'completed' && orderStatus !== 'cancelled' && (
          <div className="bg-black/50 p-2 flex gap-2 overflow-x-auto custom-scrollbar border-t border-gray-800 whitespace-nowrap z-10 relative">
            {quickReplies.map((qr, i) => (
              <button key={i} onClick={()=>{sendMessage(qr); playSound('click', isMuted);}} className="text-[10px] bg-gray-900 border border-gray-600 hover:border-orange-500 hover:text-orange-400 px-3 py-1 rounded-full uppercase font-bold tracking-wider transition-colors hover:shadow-[0_0_10px_rgba(255,69,0,0.4)]">{qr}</button>
            ))}
          </div>
        )}

        <div className="bg-gray-950 border-t border-orange-600/50 p-3 z-10 relative">
           {currentUserRole === 'COMPRADOR' && orderStatus === 'created' && (<label className="btn-ff w-full py-4 flex items-center justify-center gap-3 cursor-pointer text-lg">{isUploading ? "SUBIENDO..." : <><ImageIcon size={22} className="animate-bounce"/> REPORTAR PAGO (FOTO)</>}<input type="file" hidden accept="image/*" onChange={handleReportPayment} disabled={isUploading}/></label>)}
           {currentUserRole === 'VENDEDOR' && orderStatus === 'payment_reported' && (<button onClick={handleConfirmPayment} className="btn-ff w-full py-4 flex items-center justify-center gap-3 bg-gradient-to-r from-green-600 to-green-800 text-lg border-green-400"><CheckCircle size={22} className="animate-pulse"/> CONFIRMAR PAGO & ENVIAR DATOS</button>)}
           {currentUserRole === 'COMPRADOR' && orderStatus === 'payment_confirmed' && (<button onClick={handleFinalizeOrder} className="btn-ff w-full py-4 flex items-center justify-center gap-3 bg-gradient-to-r from-blue-600 to-blue-800 text-lg border-blue-400"><CheckSquare size={22} className="animate-pulse"/> CUENTA VERIFICADA - FINALIZAR</button>)}
           {orderStatus === 'completed' && <div className="text-center text-green-400 font-gamer uppercase p-3 border-2 border-green-500 bg-green-900/40 shadow-[0_0_20px_rgba(0,255,0,0.3)] tracking-widest text-lg animate-pulse">ORDEN COMPLETADA EXITOSAMENTE</div>}
           {orderStatus === 'cancelled' && <div className="text-center text-red-500 font-gamer uppercase p-3 border-2 border-red-600 bg-red-900/40 shadow-[0_0_20px_rgba(255,0,0,0.3)] tracking-widest text-lg animate-pulse">ORDEN CANCELADA / ANULADA</div>}
           {(orderStatus === 'created' || orderStatus === 'payment_reported' || orderStatus === 'payment_confirmed') && currentUserRole !== 'SOPORTE' && !orderData?.supportRequested && (<button onClick={requestSupport} className="w-full mt-2 py-2 text-xs font-bold uppercase text-purple-400 hover:text-white border border-purple-900 hover:border-purple-500 bg-purple-900/20 hover:bg-purple-600 transition-colors flex items-center justify-center gap-2 rounded"><Headset size={16}/> ALERTA: LLAMAR SOPORTE</button>)}
           {(orderStatus === 'created' || orderStatus === 'payment_reported' || orderStatus === 'payment_confirmed') && currentUserRole === 'SOPORTE' && (
              <div className="flex gap-2 mt-2">
                 <button onClick={adminForceCancel} className="flex-1 py-2 text-[10px] font-black uppercase text-red-300 hover:text-white border border-red-800 bg-red-900/40 hover:bg-red-700 transition-colors flex flex-col items-center justify-center gap-1 rounded hover:shadow-[0_0_15px_red]"><Gavel size={14}/> FALLAR FAVOR COMPRADOR</button>
                 <button onClick={adminForceComplete} className="flex-1 py-2 text-[10px] font-black uppercase text-green-300 hover:text-white border border-green-800 bg-green-900/40 hover:bg-green-700 transition-colors flex flex-col items-center justify-center gap-1 rounded hover:shadow-[0_0_15px_green]"><CheckCircle size={14}/> FALLAR FAVOR VENDEDOR</button>
              </div>
           )}
        </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); sendMessage(newMessage); }} className="p-3 bg-black flex gap-3 border-t border-gray-800 z-10 relative">
          ...
          />
          <button type="submit" disabled={orderStatus === 'completed' || orderStatus === 'cancelled'} className={`p-3 rounded-lg border shadow-[0_0_15px_rgba(255,69,0,0.5)] transition-transform hover:scale-110 active:scale-95 text-white ${currentUserRole === 'SOPORTE' ? 'bg-purple-600 border-purple-400 shadow-[0_0_15px_rgba(128,0,128,0.5)]' : 'bg-gradient-to-br from-orange-500 to-red-600 border-orange-400'}`}><Send size={24}/></button>
        </form>
      </div>
    </>
  );
...
const SupportDashboard = ({ user, userData, setView, showNotification, isMuted }) => {
  ...
  const banSeller = async () => { 
    ...
    await updateDoc(doc(db, 'artifacts', appId, 'users', selectedDispute.seller.id, 'profile', 'data'), { isBanned: true, banReason: "Fraude / Disputa Perdida" }); 
    ...
    <div className="animate-enter max-w-7xl mx-auto mt-10">
      <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-12 border-b-4 border-purple-800 pb-8 bg-black/40 p-8 rounded-xl backdrop-blur-sm shadow-[0_0_50px_rgba(128,0,128,0.3)] relative overflow-hidden group">
        ...
        <div className="relative z-10 text-center md:text-left">
          <h2 className="text-5xl font-gamer text-white uppercase italic text-shadow-glow drop-shadow-[0_0_20px_purple] mb-2 flex items-center justify-center md:justify-start gap-4"><Headset size={48} className="text-purple-500 animate-pulse"/> CENTRAL DE SOPORTE</h2>
          <p className="font-tech text-cyan-400 tracking-[0.4em] text-xl uppercase font-bold bg-cyan-900/30 inline-block px-4 py-1 border border-cyan-500/50"><Shield className="inline mr-2 mb-1" size={18}/> Nivel Cero: Acceso Autorizado</p>
        </div>
        <button onClick={() => {playSound('click', isMuted); setView('dashboard')}} className="btn-secondary-ff px-8 py-3 text-xl relative z-10 mt-6 md:mt-0">&lt;&lt; VOLVER</button>
      </div>

      {!selectedDispute ? (
         <div className="grid grid-cols-1 gap-6">
            {disputes.length === 0 && <div className="text-center py-20 bg-black/50 border-2 border-dashed border-gray-700 rounded-xl"><p className="text-2xl text-gray-500 font-tech uppercase tracking-widest">Sin alertas activas en el radar.</p></div>}
            {disputes.map(d => (
               <div key={d.id} className="hud-panel p-6 flex flex-col md:flex-row justify-between items-center border-purple-500/50 hover:border-purple-400 bg-gradient-to-r from-gray-900 to-black hover:scale-[1.01] transition-transform gap-4">
                  <div className="text-center md:text-left">
                    <span className="text-orange-500 font-bold bg-orange-900/30 px-2 py-1 border border-orange-500 text-xs">ORDEN #{d.orderId}</span>
                    <h3 className="text-2xl font-black text-white uppercase mt-2">{d.item.title}</h3>
                    <p className="text-gray-400 text-sm font-mono mt-1">Vendedor: {d.seller.username} | Cliente: {d.buyer.firstName} | Estado: <span className={d.status === 'cancelled' ? 'text-red-500' : 'text-yellow-500'}>{d.status}</span></p>
                  </div>
                  <div className="text-right">
                     {d.supportAgent ? ( 
                       d.supportAgent === user.uid ? 
                         <button onClick={() => {playSound('click', isMuted); setSelectedDispute(d)}} className="btn-ff bg-gradient-to-r from-purple-600 to-purple-800 border-purple-400 px-6 py-3 shadow-[0_0_20px_purple]">ATENDER MI CASO</button> : 
                         <span className="text-purple-400 font-bold bg-purple-900/40 px-4 py-2 border border-purple-500 rounded flex items-center gap-2"><Lock size={16}/> TOMADO POR {d.supportAgentName}</span> 
                     ) : ( 
                       <button onClick={() => claimCase(d.id)} className="btn-secondary-ff bg-purple-900/40 border-purple-500 text-purple-300 hover:bg-purple-600 hover:text-white px-6 py-3 shadow-[0_0_15px_rgba(128,0,128,0.3)]">ASIGNARME CASO</button> 
                     )}
                  </div>
               </div>
            ))}
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[75vh] animate-enter">
            <div className="flex flex-col gap-4 overflow-y-auto custom-scrollbar pr-2">
               <div className="bg-black/80 border-2 border-purple-600 p-6 rounded-lg shadow-[0_0_30px_rgba(128,0,128,0.2)] relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-center border-b-2 border-purple-600 pb-4 mb-6 relative z-10"><h3 className="text-3xl font-gamer text-white flex items-center gap-3"><FileWarning className="text-purple-500"/> DOSSIER DEL CASO</h3><button onClick={() => {playSound('click', isMuted); setSelectedDispute(null)}} className="text-red-500 hover:text-white bg-red-900/40 px-4 py-2 rounded font-bold hover:bg-red-600 transition-colors">CERRAR DOSSIER</button></div>
                  <div className="space-y-4 text-sm relative z-10">
                     <div className="bg-gray-900/80 p-4 rounded border-l-4 border-cyan-500 relative group"><button onClick={banSeller} className="absolute top-3 right-3 text-red-500 hover:text-white bg-red-900/40 hover:bg-red-600 px-2 py-1 rounded text-[10px] font-black uppercase flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><Ban size={12}/> BANEAR</button><h4 className="text-cyan-400 font-black mb-2 uppercase flex items-center gap-2"><User size={16}/> VENDEDOR</h4><p className="mb-1"><span className="text-gray-500">Admin:</span> {selectedDispute.seller.adminName}</p><p className="mb-1"><span className="text-gray-500">Whatsapp:</span> {selectedDispute.seller.whatsapp}</p><p><span className="text-gray-500">ID:</span> {selectedDispute.seller.idNumber}</p></div>
                     <div className="bg-gray-900/80 p-4 rounded border-l-4 border-orange-500"><h4 className="text-orange-400 font-black mb-2 uppercase flex items-center gap-2"><User size={16}/> COMPRADOR</h4><p className="mb-1"><span className="text-gray-500">Nombre:</span> {selectedDispute.buyer.firstName} {selectedDispute.buyer.lastName}</p><p className="mb-1"><span className="text-gray-500">Whatsapp:</span> {selectedDispute.buyer.whatsapp}</p><p><span className="text-gray-500">ID:</span> {selectedDispute.buyer.idNumber}</p></div>
                     <div className="bg-gray-900/80 p-4 rounded border-l-4 border-yellow-500"><h4 className="text-yellow-400 font-black mb-2 uppercase flex items-center gap-2"><Receipt size={16}/> TRANSACCIÓN</h4><p className="mb-1"><span className="text-gray-500">Item:</span> {selectedDispute.item.title}</p><p className="mb-1"><span className="text-gray-500">Total:</span> <span className="text-yellow-500 font-gamer text-xl">${selectedDispute.payment.totalUSD}</span></p><p className="mb-1"><span className="text-gray-500">Método:</span> {selectedDispute.payment.method} ({selectedDispute.payment.currency})</p><p><span className="text-gray-500">Estado:</span> <span className="bg-white/10 px-2 py-1 rounded ml-2 uppercase font-bold">{selectedDispute.status}</span></p></div>
                  </div>
               </div>
               <div className="bg-gray-950 border-2 border-gray-800 p-6 rounded-lg shadow-inner">
                  <h4 className="text-gray-400 font-black mb-4 uppercase tracking-widest flex items-center gap-2 border-b border-gray-800 pb-2"><EyeOff size={18}/> Notas Clasificadas (Solo Agentes)</h4>
                  <div className="space-y-3 mb-4 max-h-40 overflow-y-auto custom-scrollbar">
                     {(!selectedDispute.secretNotes || selectedDispute.secretNotes.length===0) && <p className="text-gray-600 text-xs italic">No hay notas en este caso.</p>}
                     {selectedDispute.secretNotes?.map((n, i) => (
                        <div key={i} className="bg-gray-900 border-l-2 border-purple-500 p-2 text-xs"><span className="text-purple-400 font-bold">[{n.agent}]:</span> <span className="text-gray-300">{n.text}</span></div>
                     ))}
                  </div>
                  <div className="flex gap-2"><input value={internalNote} onChange={e=>setInternalNote(e.target.value)} placeholder="Agregar nota interna..." className="flex-grow bg-black border border-gray-700 text-white text-sm p-2 rounded focus:border-purple-500 outline-none"/><button onClick={addSecretNote} className="bg-purple-900/50 text-purple-400 border border-purple-600 px-4 py-2 hover:bg-purple-600 hover:text-white rounded font-bold text-sm">GUARDAR</button></div>
               </div>
            </div>
            <div className="flex flex-col h-full"><ChatSystem orderId={selectedDispute.id} currentUserRole="SOPORTE" currentUserId={user.uid} orderStatus={selectedDispute.status} onUpdateStatus={()=>{}} orderData={selectedDispute} isMuted={isMuted}/></div>
         </div>
      )}
    </div>
  );
...
const OrderTrackerView = ({ setView, showNotification, isMuted }) => {
  ...
  if(foundOrder) {
    return (
      <div className="max-w-6xl mx-auto mt-10 animate-enter bg-black/80 p-8 rounded-xl border-2 border-orange-500 shadow-[0_0_60px_rgba(255,69,0,0.5)]">
        <div className="flex justify-between items-center border-b border-orange-600 pb-4 mb-6"><h2 className="text-3xl font-gamer text-white uppercase text-shadow-glow flex items-center gap-3"><Radar className="text-orange-500"/> RADAR: ORDEN #{foundOrder.orderId}</h2><button onClick={()=>{playSound('click', isMuted); setFoundOrder(null)}} className="text-red-500 font-bold hover:text-white bg-red-900/40 px-4 py-2 rounded border border-red-500 hover:bg-red-600 transition-colors">CERRAR RADAR</button></div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[65vh]">
          <div className="bg-gray-900/80 p-6 rounded border border-gray-700 overflow-y-auto custom-scrollbar shadow-inner"><h3 className="text-xl font-black text-cyan-400 border-b border-gray-700 pb-2 mb-4 tracking-widest uppercase flex items-center gap-2"><FileText size={18}/> DATOS DE OPERACIÓN</h3><p className="text-gray-300 font-mono mb-2">Producto: <span className="text-white font-bold">{foundOrder.item.title}</span></p><p className="text-gray-300 font-mono mb-2">Total a Pagar: <span className="text-yellow-500 font-bold">${foundOrder.payment.totalUSD}</span></p><p className="text-gray-300 font-mono mb-6">Vendedor Oficial: <span className="text-white font-bold">{foundOrder.seller.adminName}</span></p><div className="bg-orange-900/30 border border-orange-500 p-4 rounded text-center shadow-[0_0_15px_rgba(255,69,0,0.2)]"><p className="text-orange-400 font-bold uppercase tracking-widest text-sm mb-2"><AlertTriangle className="inline mr-2 mb-1"/> INSTRUCCIONES</p><p className="text-gray-300 text-xs">Utilice el canal a la derecha para comunicarse con el vendedor, subir el comprobante y recibir sus datos. Todo está protegido por Escrow.</p></div></div>
          <div className="flex flex-col h-full"><ChatSystem orderId={foundOrder.dbId} currentUserRole="COMPRADOR" currentUserId="GUEST_BUYER" orderStatus={foundOrder.status} onUpdateStatus={()=>{}} orderData={foundOrder} isMuted={isMuted}/></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto mt-20 animate-enter hud-panel p-10 shadow-[0_0_80px_rgba(0,255,255,0.4)] border-cyan-500 relative overflow-hidden">
       ...
       <button onClick={() => {playSound('click', isMuted); setView('home')}} className="text-gray-400 hover:text-white font-bold mb-6 relative z-10">&lt;&lt; VOLVER AL INICIO</button>
       <div className="text-center mb-10 relative z-10"><Radar size={64} className="mx-auto text-cyan-500 mb-4 animate-spin-slow"/><h2 className="text-5xl font-gamer text-white uppercase text-shadow-glow">Radar de Combate</h2><p className="text-cyan-400 font-tech tracking-widest uppercase mt-2">Rastrea y recupera tu orden de compra encriptada</p></div>
       <form onSubmit={handleTrack} className="space-y-6 relative z-10">
          <div className="input-wrapper"><Search className="w-6 h-6"/><input required value={trackId} onChange={e=>setTrackId(e.target.value)} className="input-ff w-full p-5 text-xl tracking-widest uppercase" placeholder="Nº DE ORDEN (Ej: ORD-XXXXXX)"/></div>
          <div className="input-wrapper"><Mail className="w-6 h-6"/><input type="email" required value={trackEmail} onChange={e=>setTrackEmail(e.target.value)} className="input-ff w-full p-5 text-lg" placeholder="CORREO REGISTRADO EN LA COMPRA"/></div>
          <button disabled={loading} className="w-full btn-secondary-ff border-cyan-500 bg-cyan-900/20 text-cyan-400 hover:bg-cyan-600 hover:text-white py-6 text-2xl font-black tracking-widest shadow-[0_0_30px_rgba(0,255,255,0.3)]">{loading ? "RASTREANDO SEÑAL..." : "UBICAR MI COMPRA"}</button>
       </form>
    </div>
  );
};

// --- ARMERÍA (COMPRAS DEL CLIENTE) ---
  ...
  return (
    <div className="max-w-6xl mx-auto mt-12 animate-enter">
      <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-12 border-b-4 border-blue-800 pb-8 bg-black/40 p-8 rounded-xl backdrop-blur-sm shadow-[0_0_50px_rgba(0,0,255,0.2)] gap-4">
        <div className="text-center md:text-left"><h2 className="text-5xl font-gamer text-white uppercase italic text-shadow-glow drop-shadow-[0_0_20px_blue] mb-2 flex items-center gap-4 justify-center md:justify-start"><Package size={48} className="text-blue-500"/> MI ARMERÍA</h2><p className="font-tech text-cyan-400 tracking-[0.4em] text-xl uppercase font-bold bg-cyan-900/30 inline-block px-4 py-1 border border-cyan-500/50">HISTORIAL DE COMPRAS</p></div>
        <button onClick={() => {playSound('click', isMuted); setView('home')}} className="btn-secondary-ff px-8 py-3 text-xl border-blue-500 text-blue-400 hover:bg-blue-600 hover:text-white">&lt;&lt; VOLVER</button>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {purchases.length === 0 && <div className="col-span-2 text-center py-20 bg-black/50 border-2 border-dashed border-gray-700 rounded-xl"><Package size={64} className="mx-auto text-gray-600 mb-4 opacity-50"/><p className="text-2xl text-gray-500 font-tech uppercase tracking-widest">Aún no has adquirido suministros.</p></div>}
        {purchases.map(p => (
          <div key={p.id} className="hud-panel p-6 border-blue-500/50 bg-gradient-to-br from-gray-900 to-black hover:border-blue-400 transition-all hover:scale-[1.02]">
             <div className="flex justify-between items-start mb-4 border-b border-gray-800 pb-4">
               <div><span className="text-xs font-black bg-blue-900/40 text-blue-400 border border-blue-500 px-2 py-1 rounded">ORDEN #{p.orderId}</span><h3 className="text-2xl font-black text-white uppercase mt-2">{p.item.title}</h3></div>
               <div className="text-right"><span className={`text-[10px] px-2 py-1 rounded-full font-bold uppercase shadow-[0_0_10px_currentColor] ${p.status === 'completed' ? 'bg-green-500 text-black' : p.status === 'cancelled' ? 'bg-red-600 text-white' : 'bg-yellow-500 text-black'}`}>{p.status}</span></div>
             </div>
             <div className="flex justify-between items-end">
               <div className="text-sm font-mono text-gray-400 space-y-1"><p>Fecha: {new Date(p.date).toLocaleDateString()}</p><p>Comandante: <span className="text-white">{p.seller.username}</span></p><p>Total: <span className="text-yellow-500 font-bold">${p.payment.totalUSD}</span></p></div>
               <button onClick={()=>{navigator.clipboard.writeText(p.orderId); showNotification("ID de Orden Copiado", "success"); playSound('click', isMuted)}} className="text-blue-400 text-xs font-bold hover:text-white underline">COPIAR ID</button>
             </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto mt-12 animate-enter">
      <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-12 border-b-4 border-pink-800 pb-8 bg-black/40 p-8 rounded-xl backdrop-blur-sm shadow-[0_0_50px_rgba(236,72,153,0.2)] gap-4">
        <div className="text-center md:text-left"><h2 className="text-5xl font-gamer text-white uppercase italic text-shadow-glow drop-shadow-[0_0_20px_pink] mb-2 flex items-center gap-4 justify-center md:justify-start"><Heart size={48} className="text-pink-500 fill-pink-500"/> BÓVEDA DE DESEOS</h2><p className="font-tech text-cyan-400 tracking-[0.4em] text-xl uppercase font-bold bg-cyan-900/30 inline-block px-4 py-1 border border-cyan-500/50">ARTÍCULOS MARCADOS</p></div>
        <button onClick={() => {playSound('click', isMuted); setView('home')}} className="btn-secondary-ff px-8 py-3 text-xl border-pink-500 text-pink-400 hover:bg-pink-600 hover:text-white">&lt;&lt; VOLVER AL MERCADO</button>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-8">
        {wishItems.length === 0 && <div className="col-span-4 text-center py-32 bg-black/50 border-2 border-dashed border-gray-700 rounded-xl"><Heart size={80} className="mx-auto text-gray-600 mb-6 opacity-30"/><p className="text-2xl text-gray-500 font-tech uppercase tracking-widest">No hay artículos marcados.</p></div>}
        {wishItems.map((item, index) => (
          <div key={item.id} className="hud-panel flex flex-col group h-full border-b-4 border-pink-500 animate-enter">
            <div className="relative h-60 bg-black overflow-hidden clip-path-bottom-slant">
              ...
              <button onClick={()=>removeWishlist(item.id)} className="absolute top-3 right-3 bg-black/80 p-2 rounded-full border border-pink-500 text-pink-500 hover:bg-pink-600 hover:text-white transition-colors shadow-[0_0_15px_pink] z-20"><Trash2 size={16}/></button>
              <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent opacity-90"></div>
              <div className="absolute bottom-4 left-4 flex items-center gap-2 bg-black/90 px-3 py-1 rounded border-l-2 border-yellow-500 z-20"><User size={14} className="text-yellow-500"/><span className="text-[10px] font-tech text-white uppercase">{item.adminName}</span></div>
            </div>
            <div className="p-6 flex-grow flex flex-col relative bg-gradient-to-b from-transparent to-black/90">
              ...
              <div className="mt-auto border-t border-gray-800 pt-4 flex justify-between items-end">
                <span className="text-4xl font-gamer text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]">{formatCurrency(item.discountActive ? item.discountPrice : item.price)}</span>
                <button onClick={()=>{playSound('click', isMuted); setPurchaseItem(item)}} className="bg-gradient-to-br from-yellow-400 to-yellow-600 text-black p-3 hover:scale-110 transition-transform clip-path-polygon border border-yellow-200 shadow-[0_0_15px_rgba(255,215,0,0.5)]"><ShoppingBag size={20} strokeWidth={3} /></button>
              </div>
            </div>
          </div>
        ))}
...
const SellerProfileView = ({ sellerId, onClose, onBuy, user, userData, showNotification, isMuted }) => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if(fetchedRatings.length > 0) {
         let totalScore = 0;
         fetchedRatings.forEach(r => { if(r.type === 'good') totalScore += 5; else if(r.type === 'neutral') totalScore += 3; else totalScore += 1; });
  ...
  const toggleFollow = async () => {
     ...
     if(isFollowing) { await deleteDoc(ref); setIsFollowing(false); setFollowerCount(f => f - 1); } 
     else { await setDoc(ref, { followedAt: serverTimestamp() }); setIsFollowing(true); setFollowerCount(f => f + 1); showNotification("¡COMANDANTE SEGUIDO!", "success");}
  };
  ...
  const renderStars = () => {
     return [1,2,3,4,5].map(star => (
        <Star key={star} fill={star <= Math.round(avgRating) ? "currentColor" : "none"} className={star <= Math.round(avgRating) ? "text-yellow-500 drop-shadow-[0_0_8px_yellow]" : "text-gray-600"} size={24}/>
     ));
  };
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-black/95 flex items-center justify-center p-4 overflow-auto backdrop-blur-xl">
       ...
       <div className="hud-panel p-8 md:p-12 max-w-7xl w-full relative animate-enter max-h-[90vh] overflow-y-auto custom-scrollbar shadow-[0_0_100px_rgba(255,69,0,0.4)]">
          <button onClick={()=>{playSound('click', isMuted); onClose()}} className="absolute top-6 right-6 text-gray-400 hover:text-red-500 z-50 transition-colors hover:rotate-90 duration-300"><X size={40}/></button>
          ...
          <div className="flex flex-col lg:flex-row gap-10 relative z-10">
             <div className="w-full lg:w-1/3">
                <div className="flex flex-col items-center text-center mb-6 relative group">
                   ...
                   <div className={`w-40 h-40 rounded-full border-4 ${rank.border} overflow-hidden mb-4 bg-black shadow-[0_0_50px_rgba(255,69,0,0.8)] ${rank.glow} relative z-10`}>
                      {profile.profilePicture ? <img src={profile.profilePicture} onError={(e)=>e.target.style.display='none'} className="w-full h-full object-cover hover:scale-110 transition-transform duration-500"/> : <User size={80} className="text-gray-500 m-auto mt-10"/>}
                   </div>
                   <div className={`text-xs font-black uppercase tracking-widest px-4 py-1 rounded-full border mb-4 ${rank.bg} ${rank.border} ${rank.color} ${rank.glow}`}><Award size={14} className="inline mr-1 mb-0.5"/> RANGO: {rank.rank}</div>
                   ...
                   <div className="flex items-center gap-3 justify-center flex-wrap mb-2">
                      ...
                      {isVerifiedElite && (
                         <div className="relative group/badge animate-pulse cursor-help" title={profile.isManuallyVerified ? "Verificación Oficial por Soporte" : "Verificación Automática (1000+ Ventas y 4.5+ Estrellas)"}>
                            <div className="flex items-center gap-1 bg-gradient-to-r from-red-900 to-yellow-900 border-2 border-yellow-500 px-3 py-1 rounded-full shadow-[0_0_15px_rgba(255,215,0,0.5)]"><Award className="text-yellow-400" fill="#8B0000" size={18}/><span className="text-yellow-500 font-black text-xs uppercase tracking-widest">Élite</span></div>
                         </div>
                      )}
                   </div>
                   
                   <p className="text-yellow-500 font-tech uppercase font-bold tracking-widest flex items-center gap-2 mb-4"><Users size={16}/> {followerCount} Seguidores</p>
                ...
                <div className="space-y-4 mb-8">
                   <div className="bg-black/60 p-5 border-2 border-gray-800 text-center hover:border-yellow-500 transition-all hover:scale-105 shadow-lg group"><p className="text-sm text-gray-400 uppercase font-tech tracking-wider group-hover:text-yellow-500">Ventas Completadas</p><p className="text-5xl font-gamer text-yellow-500 drop-shadow-[0_0_10px_rgba(255,215,0,0.5)] mt-2">{salesCount}</p></div>
                   <div className="bg-black/60 p-5 border-2 border-gray-800 text-center hover:border-green-500 transition-all hover:scale-105 shadow-lg group"><p className="text-sm text-gray-400 uppercase font-tech tracking-wider group-hover:text-green-400">Reputación ({avgRating.toFixed(1)})</p><div className="flex justify-center mt-3 gap-1">{renderStars()}</div></div>
                </div>
             ...
             <div className="w-full lg:w-2/3 border-l-2 border-gray-800 pl-0 lg:pl-10">
                <div className="flex gap-6 mb-8 border-b-2 border-gray-800 pb-4 overflow-x-auto custom-scrollbar">
                   <button onClick={()=>{playSound('click', isMuted); setActiveTab('arsenal')}} className={`font-tech text-2xl md:text-3xl uppercase tracking-widest flex items-center gap-3 transition-all whitespace-nowrap ${activeTab==='arsenal'?'text-white text-shadow-glow border-b-2 border-orange-500 pb-2':'text-gray-500 hover:text-gray-300'}`}><Flame className={activeTab==='arsenal' ? "text-orange-500 animate-pulse" : "text-gray-500"} size={36}/> Arsenal <span className={activeTab==='arsenal'?"text-orange-500":"text-gray-600"}>[{sellerListings.length}]</span></button>
                   <button onClick={()=>{playSound('click', isMuted); setActiveTab('references')}} className={`font-tech text-2xl md:text-3xl uppercase tracking-widest flex items-center gap-3 transition-all whitespace-nowrap ${activeTab==='references'?'text-white text-shadow-glow border-b-2 border-cyan-500 pb-2':'text-gray-500 hover:text-gray-300'}`}><Star className={activeTab==='references' ? "text-cyan-500 animate-pulse" : "text-gray-500"} size={36}/> Referencias <span className={activeTab==='references'?"text-cyan-500":"text-gray-600"}>[{profile.references?.length || 0}]</span></button>
                </div>

                {activeTab === 'arsenal' ? (
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      {sellerListings.length === 0 ? <div className="col-span-2 text-center py-20 border-2 border-dashed border-gray-700 bg-black/50"><p className="text-gray-500 italic text-xl">Este vendedor no tiene suministros activos o está de vacaciones.</p></div> : sellerListings.map((item, index) => <ProductCard key={item.id} item={item} index={index} onBuy={() => { onClose(); onBuy(item); }} onViewSeller={() => {}} />)}
                   </div>
                ) : (
                   <div className="grid grid-cols-2 md:grid-cols-3 gap-6 animate-enter">
                      {(!profile.references || profile.references.length === 0) ? <div className="col-span-full text-center py-20 border-2 border-dashed border-gray-700 bg-black/50"><p className="text-gray-500 italic text-xl font-tech uppercase tracking-widest">El comandante aún no ha subido referencias de ventas.</p></div> : profile.references.map((img, i) => (
...
// --- FORMULARIO EDICIÓN DE PERFIL (VENDEDOR) ---
const EditProfileForm = ({ user, userData, setView, showNotification, isMuted }) => {
  const [formData, setFormData] = useState({ 
     firstName: userData?.firstName||'', lastName: userData?.lastName||'', whatsapp: userData?.whatsapp||'', 
     publicUsername: userData?.publicUsername||'', adminName: userData?.adminName||'',
     profilePicture: userData?.profilePicture||'', references: userData?.references||[]
  });
  const [loading, setLoading] = useState(false);

  const handleUpdate = async (e) => {
     e.preventDefault(); setLoading(true); playSound('click', isMuted);
     try {
        await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'data'), formData);
        showNotification("PERFIL ACTUALIZADO CON ÉXITO", "success"); playSound('success', isMuted); setView('dashboard');
     } catch (e) { showNotification("ERROR AL ACTUALIZAR", "error"); playSound('error', isMuted);} finally { setLoading(false); }
  };
  ...
  const removeRef = (index) => {
     playSound('click', isMuted);
     const newRefs = [...formData.references]; newRefs.splice(index, 1);
  ...
  return (
    <div className="max-w-4xl mx-auto mt-12 animate-enter hud-panel p-10 md:p-14 shadow-[0_0_80px_rgba(255,69,0,0.5)]">
       <div className="flex flex-col md:flex-row justify-between items-center border-b-2 border-orange-600 pb-6 mb-8 gap-4">
          <h2 className="text-4xl font-gamer text-white uppercase text-shadow-glow flex items-center gap-3"><Edit className="text-orange-500"/> MODIFICAR PERFIL</h2>
          <button onClick={()=>{playSound('click', isMuted); setView('dashboard')}} className="text-gray-400 hover:text-white font-bold">&lt;&lt; CANCELAR</button>
       </div>
       ...
       <form onSubmit={handleUpdate} className="space-y-8">
          <div className="flex flex-col md:flex-row gap-8 items-center bg-black/50 p-6 rounded-lg border border-gray-800">
             <div className="relative group w-32 h-32 rounded-full border-4 border-orange-500 bg-black overflow-hidden shadow-[0_0_20px_rgba(255,69,0,0.5)] flex-shrink-0">
                {formData.profilePicture ? <img src={formData.profilePicture} onError={(e)=>e.target.style.display='none'} className="w-full h-full object-cover" /> : <User size={64} className="text-gray-500 m-auto mt-8"/>}
             ...
             <div className="text-center md:text-left">
                <h3 className="text-2xl font-tech text-white uppercase tracking-widest text-shadow-glow mb-2 flex items-center gap-2 justify-center md:justify-start"><ImageIcon size={20} className="text-orange-500"/> Identidad Visual</h3>
                <p className="text-gray-400 text-sm font-mono leading-relaxed">Sube tu logo o avatar personalizado. Si no subes ninguno, aparecerá un ícono anónimo para proteger tu privacidad.</p>
             </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-black/40 p-6 border border-gray-800 rounded">
             <div className="input-wrapper"><User className="w-6 h-6"/><input name="firstName" value={formData.firstName} onChange={e=>setFormData({...formData, firstName:e.target.value})} className="input-ff w-full p-4 text-lg" placeholder="Nombres" required/></div>
             <div className="input-wrapper"><User className="w-6 h-6"/><input name="lastName" value={formData.lastName} onChange={e=>setFormData({...formData, lastName:e.target.value})} className="input-ff w-full p-4 text-lg" placeholder="Apellidos" required/></div>
             <div className="input-wrapper"><Smartphone className="w-6 h-6"/><input name="whatsapp" value={formData.whatsapp} onChange={e=>setFormData({...formData, whatsapp:e.target.value})} className="input-ff w-full p-4 text-lg" placeholder="Whatsapp" required/></div>
             <div className="input-wrapper"><UserCheck className="w-6 h-6"/><input name="publicUsername" value={formData.publicUsername} onChange={e=>setFormData({...formData, publicUsername:e.target.value})} className="input-ff w-full p-4 text-lg" placeholder="Alias Público" required/></div>
             <div className="input-wrapper md:col-span-2"><Shield className="w-6 h-6"/><input name="adminName" value={formData.adminName} onChange={e=>setFormData({...formData, adminName:e.target.value})} className="input-ff w-full p-4 text-lg" placeholder="Nombre Real / Empresa" required/></div>
          </div>

          <div className="mt-8 border-t-2 border-dashed border-gray-800 pt-8">
             <h3 className="text-xl font-tech text-cyan-400 uppercase tracking-widest flex items-center gap-3 mb-2 text-shadow-glow"><Star size={24}/> Referencias de Ventas y Reputación</h3>
             <p className="text-gray-400 text-sm font-mono mb-6 leading-relaxed">Sube capturas de pantalla de tus ventas exitosas, conversaciones con clientes satisfechos o reputación previa. Esto generará confianza masiva en los nuevos compradores.</p>
                   ...
                   <div className="flex flex-wrap gap-4 mt-8 justify-center relative z-10 bg-black/80 p-6 rounded border border-cyan-900/50 shadow-[inset_0_0_20px_rgba(0,255,255,0.1)]">
                      {formData.references.map((img, i) => (
                         <div key={i} className="relative group/img">
                            ...
                            <button type="button" onClick={() => removeRef(i)} className="absolute -top-3 -right-3 bg-red-600 border-2 border-black text-white p-1 rounded-full opacity-0 group-hover/img:opacity-100 transition-opacity hover:scale-125 shadow-lg"><X size={14}/></button>
                         </div>
                      ))}
                   </div>
                )}
          ...
          <button disabled={loading} className="w-full btn-ff py-6 text-3xl mt-8 shadow-[0_0_40px_rgba(255,69,0,0.6)] hover:shadow-[0_0_60px_rgba(255,69,0,1)] tracking-widest">{loading ? "ACTUALIZANDO SISTEMA..." : "GUARDAR CAMBIOS"}</button>
       </form>
    </div>
  );
};
...
const Dashboard = ({ user, userData, listings, setView, showNotification, setViewSellerId, isMuted }) => {
  ...
  useEffect(() => { 
    const unsub = onSnapshot(query(collection(db, 'artifacts', appId, 'public', 'data', 'orders'), where('seller.id', '==', user.uid)), (snap) => setOrders(snap.docs.map(d => ({id:d.id, ...d.data()})))); 
    return () => unsub(); 
  }, [user.uid]);
  ...
  const pauseAll = async () => { 
    ...
    for(let item of myListings) { 
      if(item.isActive !== false) await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'listings', item.id), { isActive: false }); 
    } 
    ...
    <div className="animate-enter max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-8 border-b-4 border-gray-800 pb-8 bg-black/40 p-8 rounded-xl backdrop-blur-sm relative overflow-hidden group">
        ...
        <div className="text-center md:text-left relative z-10 w-full md:w-auto">
          ...
          <div className="flex flex-wrap items-center gap-4 justify-center md:justify-start">
            <p className="font-tech text-cyan-400 tracking-[0.4em] text-xl uppercase font-bold bg-cyan-900/30 px-4 py-1 border border-cyan-500/50">Comandante {userData?.adminName}</p>
            <span className={`text-xs font-black uppercase tracking-widest px-4 py-1.5 rounded-full border ${rank.bg} ${rank.border} ${rank.color} ${rank.glow}`}><Award size={14} className="inline mr-1 mb-0.5"/> RANGO: {rank.rank}</span>
          </div>
          <div className="mt-6 max-w-md w-full">
            <div className="flex justify-between text-xs font-tech font-bold text-gray-400 mb-1"><span>XP ACTUAL: {xp}</span><span>PRÓXIMO NIVEL: {nextLevelXp} XP</span></div>
            <div className="xp-bar-container"><div className="xp-bar-fill" style={{width: `${xpPercentage}%`}}></div></div>
          </div>
        </div>
        <div className="flex flex-wrap gap-4 mt-8 md:mt-0 relative z-10 justify-center">
          <button onClick={() => {playSound('click', isMuted); setViewSellerId(user.uid)}} className="btn-secondary-ff px-6 py-4 flex items-center gap-2 border-cyan-500 bg-cyan-900/20 text-cyan-400 hover:bg-cyan-600 hover:text-white font-bold"><Eye size={20}/> MI PERFIL PÚBLICO</button>
          <button onClick={() => {playSound('click', isMuted); setView('edit-profile')}} className="btn-secondary-ff px-6 py-4 flex items-center gap-2 font-bold"><Edit size={20}/> EDITAR PERFIL</button>
          <button onClick={() => {playSound('click', isMuted); setView('create')}} className="btn-ff px-8 py-4 flex items-center gap-3 text-xl shadow-[0_0_30px_rgba(255,69,0,0.6)]"><Plus size={24} className="animate-spin-slow" /> NUEVO DESPLIEGUE</button>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <div className="hud-panel p-6 bg-gradient-to-br from-green-900/40 to-black border-green-500"><h3 className="text-green-400 font-tech uppercase tracking-widest text-sm mb-2 flex items-center gap-2"><Activity size={18}/> Ingresos Totales</h3><p className="text-5xl font-gamer text-white drop-shadow-[0_0_15px_green]">${totalRevenue.toFixed(2)}</p></div>
        <div className="hud-panel p-6 bg-gradient-to-br from-yellow-900/40 to-black border-yellow-500"><h3 className="text-yellow-400 font-tech uppercase tracking-widest text-sm mb-2 flex items-center gap-2"><RefreshCw size={18}/> Órdenes en Proceso</h3><p className="text-5xl font-gamer text-white drop-shadow-[0_0_15px_yellow]">{pendingCount}</p></div>
        <div className="hud-panel p-6 bg-gradient-to-br from-blue-900/40 to-black border-blue-500"><h3 className="text-blue-400 font-tech uppercase tracking-widest text-sm mb-2 flex items-center gap-2"><CheckSquare size={18}/> Ventas Cerradas</h3><p className="text-5xl font-gamer text-white drop-shadow-[0_0_15px_blue]">{completedSales}</p></div>
      </div>
      ...
      <div className="mt-16">
        <div className="flex flex-col md:flex-row justify-between items-center bg-black/50 p-4 border-l-4 border-yellow-500 rounded mb-8 gap-4">
          <h3 className="font-tech text-3xl text-white uppercase tracking-[0.2em] flex items-center gap-4 text-shadow-glow"><Trophy className="text-yellow-500 drop-shadow-[0_0_10px_yellow]" size={36}/> Inventario Activo <span className="text-gray-500">[{myListings.length}]</span></h3>
          <button onClick={pauseAll} className="text-red-400 border border-red-500 bg-red-900/30 hover:bg-red-600 hover:text-white px-4 py-2 font-bold tracking-widest uppercase rounded flex items-center gap-2 text-sm"><Ban size={16}/> Pausar Todo (Vacaciones)</button>
        </div>
          ...
          {myListings.map(item => { 
            ...
            return (
              <div key={item.id} className={`hud-panel p-6 flex flex-col md:flex-row items-center gap-8 hover:bg-gray-900/80 transition-all shadow-lg ${!isActive && 'opacity-60 grayscale'}`}>
                <div className="w-32 h-32 bg-black border-2 border-orange-600 shrink-0 overflow-hidden relative group"><img src={item.images?.[0]} onError={(e)=>e.target.style.display='none'} className="w-full h-full object-cover group-hover:scale-125 transition-transform duration-700" /></div>
                <div className="flex-grow text-center md:text-left"><h4 className="font-black font-tech text-3xl text-white uppercase tracking-wider mb-2 drop-shadow-md">{item.title}</h4><span className="text-yellow-500 font-gamer text-4xl drop-shadow-[0_0_10px_rgba(255,215,0,0.5)]">{formatCurrency(item.price)}</span> {!isActive && <span className="ml-4 text-xs font-black bg-red-900 text-red-400 px-2 py-1 rounded">PAUSADO</span>}</div>
                <div className="flex flex-wrap justify-center gap-3">
                   <button onClick={() => togglePause(item.id, isActive)} className={`px-4 py-3 font-bold border-2 transition-all flex items-center gap-2 ${isActive ? 'bg-orange-900/40 text-orange-400 border-orange-500 hover:bg-orange-500 hover:text-white' : 'bg-green-900/40 text-green-400 border-green-500 hover:bg-green-500 hover:text-white'}`}>{isActive ? <PauseCircle size={18}/> : <PlayCircle size={18}/>} {isActive ? 'PAUSAR' : 'ACTIVAR'}</button>
                   <button onClick={() => {playSound('click', isMuted); setView(`edit-${item.id}`)}} className="btn-secondary-ff px-4 py-3 flex items-center gap-2 font-bold"><Edit size={18}/> EDITAR</button>
                </div>
              </div>
            )
          })} 
          {myListings.length === 0 && <div className="text-center py-20 bg-black/50 border-2 border-dashed border-gray-700 rounded-xl"><p className="text-2xl text-gray-500 font-tech uppercase tracking-widest">No hay suministros desplegados.</p></div>}
        </div>
      </div>
    </div>
  );
...
// --- GESTIÓN DE MÉTODOS DE PAGO Y ÓRDENES (VENDEDOR) ---
const PaymentMethodsManager = ({ user, showNotification, isMuted }) => {
  ...
  useEffect(() => { 
    if (!user) return; 
    const unsub = onSnapshot(collection(db, 'artifacts', appId, 'users', user.uid, 'paymentMethods'), (snap) => setMethods(snap.docs.map(d => ({id: d.id, ...d.data()})))); 
    return () => unsub(); 
  }, [user]);

  const handleAdd = async (e) => { 
    e.preventDefault(); playSound('click', isMuted); 
  ...
  return (
    <div className="hud-panel p-8 mt-12 shadow-[0_0_40px_rgba(0,255,255,0.1)]">
      ...
      <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10 bg-black/60 p-6 border-2 border-gray-800 rounded">
         ...
         <button disabled={loading} className="btn-ff py-4 md:col-span-4 flex justify-center items-center gap-3 text-xl tracking-widest"><Plus size={24}/> AGREGAR A BÓVEDA</button>
      </form>
        ...
        {orders.map(order => (
          <div key={order.id} className="bg-gradient-to-r from-gray-900 to-black border-2 border-gray-800 p-6 flex flex-col md:flex-row justify-between items-center gap-6 hover:border-green-500 transition-all hover:scale-[1.01] shadow-lg rounded">
            ...
            <div className="text-right flex flex-col items-end">
              ...
              <span className={`text-xs px-4 py-2 rounded-full font-black uppercase tracking-widest shadow-[0_0_15px_currentColor] ${order.status === 'completed' ? 'bg-green-500 text-black' : order.status === 'cancelled' ? 'bg-red-600 text-white' : 'bg-yellow-500 text-black'}`}>{order.status === 'created' ? 'NUEVA' : order.status === 'payment_reported' ? 'PAGO REPORTADO' : order.status === 'payment_confirmed' ? 'DATOS ENVIADOS' : order.status === 'cancelled' ? 'CANCELADA' : 'COMPLETADA'}</span>
            </div>
            <button onClick={() => {playSound('click', isMuted); setSelectedOrder(order)}} className="btn-secondary-ff px-6 py-4 text-sm flex gap-3 font-bold tracking-widest border-cyan-500 bg-cyan-900/20 hover:bg-cyan-600 hover:text-white"><MessageSquare size={20}/> GESTIONAR</button>
          </div>
        ))}
      </div>
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="hud-panel p-8 max-w-6xl w-full h-[85vh] flex flex-col relative shadow-[0_0_80px_rgba(0,255,255,0.3)] border-cyan-600">
            <button onClick={() => {playSound('click', isMuted); setSelectedOrder(null)}} className="absolute top-6 right-6 text-gray-400 hover:text-white transition-colors hover:rotate-90 duration-300 z-50"><X size={40}/></button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-grow overflow-hidden">
              <div className="overflow-y-auto pr-4 space-y-6 text-base custom-scrollbar">
                <div className="bg-gray-900/80 p-4 rounded border-l-4 border-orange-500 shadow-md">
                  <h4 className="text-orange-400 font-black mb-2 uppercase tracking-widest flex items-center gap-2"><User size={18}/> DATOS DEL CLIENTE</h4>
                ...
                <div className="bg-gray-900/80 p-4 rounded border-l-4 border-cyan-500 shadow-md">
                  <h4 className="text-cyan-400 font-black mb-2 uppercase tracking-widest flex items-center gap-2"><Receipt size={18}/> DATOS FINANCIEROS</h4>
                  ...
                  {selectedOrder.payment.currency === 'VES' && <div className="mt-4 p-4 bg-green-900/20 border border-green-500 rounded"><p className="text-sm text-green-400 mb-1">Tasa: {selectedOrder.payment.rateUsed}</p><p className="text-2xl font-black text-green-400">Total VES: {formatCurrency(selectedOrder.payment.totalVES, 'VES')}</p></div>}
                </div>
              </div>
              <div className="flex flex-col h-full"><ChatSystem orderId={selectedOrder.id} currentUserRole="VENDEDOR" currentUserId={user.uid} orderStatus={selectedOrder.status} onUpdateStatus={updateOrderStatus} orderData={selectedOrder} isMuted={isMuted}/></div>
            </div>
          </div>
        </div>
      )}
...
// --- CREACIÓN DE ARSENAL (LISTING FORM) ---
const ListingForm = ({ user, userData, setView, showNotification, mode = 'create', editId = null, listings = [], isMuted }) => {
  ...
  useEffect(() => {
    if (mode === 'edit' && editId) { const item = listings.find(l => l.id === editId); if (item) setData({ title: item.title, price: item.price, description: item.description, images: item.images || [] }); }
  }, [mode, editId, listings]);

  const handleFileChange = async (e) => {
    playSound('click', isMuted);
    const files = Array.from(e.target.files); const newImages = await Promise.all(files.map(f => compressImage(f)));
  ...
  const handleSave = async (e) => {
    ...
    try {
       ...
       if(mode === 'create') await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'listings'), { ...payload, createdAt: serverTimestamp(), isActive: true });
       else await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'listings', editId), payload);
       showNotification("ARSENAL PUBLICADO CON ÉXITO", "success"); playSound('success', isMuted); setView('dashboard');
    } catch(e) { showNotification("ERROR DE TRANSMISIÓN", "error"); playSound('error', isMuted); } finally { setLoading(false); }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto hud-panel p-10 md:p-14 mt-12 animate-enter shadow-[0_0_80px_rgba(255,69,0,0.5)] border-2 border-orange-500 bg-black/90 backdrop-blur-xl">
       <div className="flex flex-col md:flex-row justify-between items-center mb-12 border-b-2 border-orange-600 pb-6"><h2 className="text-4xl md:text-5xl font-gamer text-white uppercase italic text-shadow-glow flex items-center gap-4"><Flame size={48} className="text-orange-500 animate-pulse"/> {mode === 'create' ? 'Inicializando Despliegue' : 'Modificando Activo'}</h2><button onClick={() => {playSound('click', isMuted); setView('dashboard')}} className="mt-6 md:mt-0 text-gray-400 hover:text-red-500 transition-all hover:rotate-90 hover:scale-125 duration-300 bg-black/50 p-2 rounded-full border border-gray-700 hover:border-red-500"><X size={36} /></button></div>
       <form onSubmit={handleSave} className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
             <div className="md:col-span-2"><label className="block text-cyan-400 font-bold mb-2 uppercase tracking-widest text-sm">Título del Suministro</label><input placeholder="Ej: Cuenta Sakura Veterana" className="input-ff w-full p-5 text-2xl" value={data.title} onChange={e => setData({...data, title: e.target.value})} required /></div>
             <div><label className="block text-yellow-500 font-bold mb-2 uppercase tracking-widest text-sm">Valor (USD)</label><input type="number" placeholder="Ej: 150" className="input-ff w-full p-5 text-yellow-500 font-gamer text-3xl shadow-[inset_0_0_15px_rgba(255,215,0,0.2)]" value={data.price} onChange={e => setData({...data, price: e.target.value})} required /></div>
          </div>
          <div><label className="block text-cyan-400 font-bold mb-2 uppercase tracking-widest text-sm">Informe Detallado de Combate</label><textarea placeholder="Especificar armas evolutivas, pases élite..." className="input-ff w-full p-5 h-48 text-lg font-mono" value={data.description} onChange={e => setData({...data, description: e.target.value})} required /></div>
          <div className="border-4 border-dashed border-gray-700 bg-black/50 p-10 text-center hover:border-orange-500 hover:bg-orange-900/10 transition-all group rounded-xl shadow-inner cursor-pointer relative"><Camera size={64} className="mx-auto mb-4 text-gray-600 group-hover:text-orange-500 transition-colors drop-shadow-md group-hover:drop-shadow-[0_0_15px_orange] group-hover:scale-110 duration-300"/><label className="cursor-pointer text-lg font-black uppercase tracking-widest text-gray-400 group-hover:text-white relative z-10 w-full block">[ + Click Para Anexar Evidencia + ] <input type="file" hidden multiple accept="image/*" onChange={handleFileChange}/></label>
             {data.images.length > 0 && (<div className="flex flex-wrap gap-4 mt-8 justify-center relative z-10 bg-black/80 p-6 rounded border border-gray-800">{data.images.map((img, i) => (<div key={i} className="relative group/img"><img src={img} className="w-24 h-24 object-cover border-2 border-orange-500 rounded shadow-[0_0_15px_rgba(255,69,0,0.5)] group-hover/img:scale-150 transition-transform duration-300 z-10 relative"/>{i === 0 && <span className="absolute -top-3 -left-3 bg-red-600 text-white text-[10px] font-black px-2 py-1 rounded border border-white z-20 uppercase">Portada</span>}</div>))}</div>)}
          </div>
          <button disabled={loading} className="w-full btn-ff py-6 text-3xl shadow-[0_0_60px_rgba(255,69,0,0.6)] hover:shadow-[0_0_80px_rgba(255,69,0,1)] tracking-[0.2em] relative"><span className="relative z-10 flex items-center justify-center gap-4">{loading ? <><RefreshCw className="animate-spin"/> TRANSMITIENDO DATOS...</> : <><Upload className="animate-bounce"/> PUBLICAR ARSENAL AHORA</>}</span></button>
       </form>
    </div>
  );
};

// --- FORMULARIOS AUTH (LOGIN / REGISTRO TOTAL) ---
const LoginForm = ({ setView, showNotification, isMuted }) => {
  ...
  return (
    <div className="max-w-md mx-auto mt-20 relative animate-enter">
      <div className="absolute -left-64 top-[-50px] hidden xl:block animate-floatExtreme pointer-events-none z-[8]"><img src={proxyImg('https://freelogopng.com/images/all_img/1664286161free-fire-characters-png.png')} onError={(e)=>e.target.style.display='none'} className="h-[500px] opacity-70 drop-shadow-[0_0_50px_rgba(255,69,0,0.8)] mix-blend-screen" /></div>
      ...
      <div className="hud-panel p-10 md:p-14 relative z-10 bg-black/95 shadow-[0_0_80px_rgba(255,69,0,0.8)] border-2 border-orange-500">
        {showRecover ? (
          <div className="animate-enter"><h3 className="text-3xl font-gamer text-white uppercase text-center mb-8 text-shadow-glow">Recuperar Acceso</h3><div className="bg-cyan-900/20 border-l-4 border-cyan-500 p-4 mb-6 shadow-[0_0_15px_rgba(0,255,255,0.2)]"><p className="text-sm text-cyan-400 font-bold tracking-wide"><span className="text-white">NOTA:</span> El enlace llegará a su correo. Si no lo visualiza, <span className="text-yellow-400">revise la carpeta de Spam.</span></p></div><form onSubmit={handleRecover} className="space-y-8"><div className="input-wrapper"><Mail className="w-6 h-6"/><input type="email" required className="input-ff w-full p-5 text-lg" placeholder="CORREO REGISTRADO" value={email} onChange={(e) => setEmail(e.target.value)} /></div><div className="flex gap-4"><button type="button" onClick={() => setShowRecover(false)} className="flex-1 btn-secondary-ff py-4 text-lg font-bold">CANCELAR</button><button type="submit" className="flex-1 btn-ff py-4 text-lg shadow-[0_0_20px_rgba(255,69,0,0.5)]">ENVIAR</button></div></form></div>
        ) : (
          <div className="animate-enter"><div className="text-center mb-12 relative group"><div className="w-28 h-28 mx-auto bg-black border-4 border-orange-500 rounded-full flex items-center justify-center mb-6 shadow-[0_0_50px_rgba(255,69,0,0.8)] relative z-10 transform group-hover:rotate-12 transition-transform duration-500"><Lock size={48} className="text-orange-500 drop-shadow-[0_0_10px_orange]" /></div><h2 className="text-5xl font-gamer text-white uppercase italic drop-shadow-[0_5px_10px_rgba(0,0,0,0.8)] text-shadow-glow relative z-10">Acceso Oficial</h2></div><form onSubmit={handleSubmit} className="space-y-10"><div className="group"><label className="block text-sm font-black text-cyan-400 mb-3 font-tech uppercase tracking-[0.2em] drop-shadow-md">Agente</label><div className="input-wrapper"><Mail className="w-6 h-6"/><input type="email" required className="input-ff w-full p-5 text-xl tracking-wider" placeholder="AGENTE@TECNOBYTE.COM" value={email} onChange={(e) => setEmail(e.target.value)} /></div></div><div className="group"><label className="block text-sm font-black text-cyan-400 mb-3 font-tech uppercase tracking-[0.2em] drop-shadow-md">Código de Seguridad</label><div className="input-wrapper"><Key className="w-6 h-6"/><input type="password" required className="input-ff w-full p-5 text-xl tracking-[0.3em]" placeholder="••••••••••••" value={password} onChange={(e) => setPassword(e.target.value)} /></div><div className="text-right mt-3"><button type="button" onClick={() => setShowRecover(true)} className="text-xs text-yellow-500 hover:text-white uppercase font-black tracking-widest transition-colors hover:underline">¿Olvidó su contraseña?</button></div></div><button type="submit" className="w-full btn-ff py-6 text-3xl mt-10 shadow-[0_10px_40px_rgba(255,0,0,0.6)] tracking-widest relative overflow-hidden"><span className="relative z-10 flex items-center justify-center gap-3"><Zap className="animate-pulse"/> AUTENTICAR</span></button></form><div className="mt-10 text-center border-t-2 border-gray-800 pt-8 relative"><button onClick={() => setView('register')} className="text-gray-400 hover:text-yellow-400 text-sm font-tech uppercase tracking-[0.3em] font-bold transition-all hover:scale-110 drop-shadow-md">&gt;&gt; Solicitar Permisos de Venta</button></div></div>
        )}
      </div>
    </div>
  );
...
const RegisterForm = ({ setView, showNotification, isMuted }) => {
  ...
  const handleRegister = async (e) => { 
     ...
     setLoading(true); playSound('click', isMuted); 
     try { 
        ...
        showNotification("REGISTRO COMPLETADO", "success"); playSound('success', isMuted); setView('dashboard'); 
     } catch (e) { showNotification(e.message, "error"); playSound('error', isMuted);} finally { setLoading(false); } 
  };

  return (
    <div className="max-w-5xl mx-auto mt-16 relative animate-enter">
      <div className="absolute -right-64 top-0 hidden xl:block animate-floatExtreme pointer-events-none z-[8]" style={{ animationDirection: 'reverse' }}><img src={proxyImg('https://freelogopng.com/images/all_img/1664285810free-fire-character-png.png')} onError={(e)=>e.target.style.display='none'} alt="decor right" className="h-[600px] opacity-60 drop-shadow-[0_0_50px_rgba(255,69,0,0.8)]" /></div>
          ...
          {step === 1 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-enter">
              ...
              <div className="md:col-span-2 bg-gradient-to-r from-gray-900 to-black p-8 border-2 border-gray-800 rounded mt-4 shadow-inner">
                 ...
                 <div className="flex flex-wrap gap-4 mt-6 text-xs text-gray-500 uppercase font-bold tracking-widest bg-black/50 p-4 rounded border border-gray-800"><span className={`flex items-center gap-1 ${passStrength.length ? "text-green-500" : ""}`}>{passStrength.length && <CheckCircle size={12}/>} 8+ Caracteres</span><span className={`flex items-center gap-1 ${passStrength.upper ? "text-green-500" : ""}`}>{passStrength.upper && <CheckCircle size={12}/>} Mayúscula</span><span className={`flex items-center gap-1 ${passStrength.num ? "text-green-500" : ""}`}>{passStrength.num && <CheckCircle size={12}/>} Número</span><span className={`flex items-center gap-1 ${passStrength.special ? "text-green-500" : ""}`}>{passStrength.special && <CheckCircle size={12}/>} Símbolo</span></div>
              </div>
              <button type="button" onClick={handleNextStep} className="btn-ff py-6 md:col-span-2 text-2xl font-black tracking-[0.2em] mt-4 shadow-[0_0_40px_rgba(255,69,0,0.5)] flex justify-center items-center gap-3">SIGUIENTE FASE <Zap/></button>
            </div>
          )}
          {step === 2 && (
            <div className="space-y-8 animate-enter">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <div className="bg-gradient-to-b from-gray-900 to-black p-10 border-2 border-gray-800 text-center relative group hover:border-orange-500 transition-colors rounded-lg shadow-lg">
                    <ScanFace size={64} className="mx-auto mb-6 text-gray-600 group-hover:text-orange-500"/><p className="text-lg font-black mb-4 uppercase tracking-[0.2em] text-white">FOTOGRAFÍA EN VIVO *</p>
                    <label className="text-sm btn-secondary-ff p-4 cursor-pointer block font-bold tracking-widest hover:bg-orange-600 hover:text-white hover:border-orange-400">{kycData.selfie ? "REEMPLAZAR CAPTURA" : "ACTIVAR CÁMARA FRONTAL"} <input type="file" hidden accept="image/*" capture="user" onChange={e => handleKyc('selfie', e.target.files[0])}/></label>
                 </div>
                 <div className="bg-gradient-to-b from-gray-900 to-black p-10 border-2 border-gray-800 text-center relative group hover:border-cyan-500 transition-colors rounded-lg shadow-lg">
                    <FileText size={64} className="mx-auto mb-6 text-gray-600 group-hover:text-cyan-500"/><p className="text-lg font-black mb-4 uppercase tracking-[0.2em] text-white">DOCUMENTO DNI/ID *</p>
                    <label className="text-sm btn-secondary-ff p-4 cursor-pointer block font-bold tracking-widest hover:bg-cyan-600 hover:text-white hover:border-cyan-400">{kycData.docFront ? "REEMPLAZAR ESCANEO" : "ESCANEAR DOCUMENTO"} <input type="file" hidden accept="image/*" onChange={e => handleKyc('docFront', e.target.files[0])}/></label>
                 </div>
              </div>
              <div className="bg-black/50 p-8 border border-gray-800 rounded space-y-6"><div className="input-wrapper"><UserCheck className="w-6 h-6"/><input name="publicUsername" placeholder="Alias Público (Ej: GhostKiller) *" className="input-ff p-5 w-full text-xl" onChange={handleChange} /></div><div className="input-wrapper"><Shield className="w-6 h-6"/><input name="adminName" placeholder="Nombre Real / Empresa *" className="input-ff p-5 w-full text-xl" onChange={handleChange} /></div></div>
              <label className="flex items-center gap-4 text-base text-gray-300 bg-orange-900/20 p-6 border-2 border-orange-500/50 rounded-lg cursor-pointer hover:bg-orange-900/40 hover:border-orange-400 transition-all shadow-[0_0_20px_rgba(255,69,0,0.1)] group"><input type="checkbox" onChange={e => setLiveness(e.target.checked)} className="w-6 h-6 accent-orange-600 cursor-pointer"/> <span className="font-bold tracking-wide uppercase group-hover:text-white">Confirmo que soy persona real (Prueba de Vida).</span></label>
              <div className="flex gap-6 mt-8"><button type="button" onClick={() => setStep(1)} className="flex-[1] btn-secondary-ff py-5 font-bold text-xl tracking-widest">&lt;&lt; ATRÁS</button><button disabled={loading} className="flex-[2] btn-ff py-5 text-2xl shadow-[0_0_50px_rgba(255,69,0,0.8)] animate-pulse font-black tracking-[0.2em]">{loading ? "ENCRIPTANDO DATOS..." : "FINALIZAR REGISTRO"}</button></div>
            </div>
          )}
        </form>
      </div>
...
// --- AVISOS LEGALES ---
const TermsView = ({ setView }) => (<div className="max-w-4xl mx-auto mt-12 animate-enter hud-panel p-10 shadow-[0_0_80px_rgba(255,69,0,0.5)] bg-black/90"><button onClick={() => setView('home')} className="text-gray-400 hover:text-white font-bold mb-6 flex items-center gap-2">&lt;&lt; VOLVER AL INICIO</button><h2 className="text-4xl font-gamer text-white uppercase text-shadow-glow border-b-2 border-orange-600 pb-4 mb-8">TÉRMINOS Y CONDICIONES DE SERVICIO</h2><div className="space-y-6 text-gray-300 font-mono text-sm leading-relaxed"><p>Bienvenido a Nexus Station. Al utilizar nuestra plataforma, aceptas los siguientes términos establecidos por <strong>TecnoByte LLC</strong>.</p><h3 className="text-orange-500 font-bold text-lg mt-4">1. Naturaleza del Servicio</h3><p>Nexus Station funciona estrictamente como una plataforma intermediaria (Escrow P2P). Proveemos el canal de comunicación seguro y la infraestructura tecnológica para conectar compradores y vendedores.</p><h3 className="text-orange-500 font-bold text-lg mt-4">2. Verificación de Identidad (KYC)</h3><p>Todo vendedor debe proveer información real y comprobable (Identificación Oficial y Prueba de Vida Biográfica). Falsificar esta información resultará en el bloqueo permanente de la cuenta y reporte a las autoridades competentes.</p><h3 className="text-orange-500 font-bold text-lg mt-4">3. Política de Transacciones y Reembolsos</h3><p>Los fondos deben ser confirmados antes de liberar cualquier credencial. Una vez que el comprador hace clic en "CUENTA VERIFICADA", la transacción se considera <strong>FINALIZADA e IRREVERSIBLE</strong>. No se emitirán reembolsos posteriores a esta acción bajo ninguna circunstancia.</p><h3 className="text-orange-500 font-bold text-lg mt-4">4. Sistema de Disputas</h3><p>En caso de anomalías, tanto el vendedor como el comprador pueden activar el botón "Llamar Soporte". Un agente oficial de TecnoByte LLC intervendrá, revisará el historial del chat encriptado y tomará una decisión final vinculante.</p></div></div>);
const LegalView = ({ setView }) => (<div className="max-w-4xl mx-auto mt-12 animate-enter hud-panel p-10 shadow-[0_0_80px_rgba(255,69,0,0.5)] bg-black/90"><button onClick={() => setView('home')} className="text-gray-400 hover:text-white font-bold mb-6 flex items-center gap-2">&lt;&lt; VOLVER AL INICIO</button><h2 className="text-4xl font-gamer text-white uppercase text-shadow-glow border-b-2 border-orange-600 pb-4 mb-8">AVISO LEGAL Y DESCARGO DE RESPONSABILIDAD</h2><div className="space-y-6 text-gray-300 font-mono text-sm leading-relaxed"><h3 className="text-orange-500 font-bold text-lg">1. Propiedad Intelectual</h3><p><strong>TecnoByte LLC</strong> y el portal Nexus Station NO están afiliados, asociados, autorizados, respaldados ni conectados oficialmente de ninguna manera con Garena, Garena Free Fire, ni ninguna de sus subsidiarias o afiliados. Los nombres, marcas comerciales, emblemas e imágenes registrados pertenecen exclusivamente a sus respectivos propietarios.</p><h3 className="text-orange-500 font-bold text-lg mt-4">2. Responsabilidad de la Empresa</h3><p>TecnoByte LLC proporciona la plataforma y el sistema de custodia. No somos propietarios de los artículos digitales listados en el mercado. La responsabilidad de la veracidad y legalidad de los artículos recae enteramente sobre los usuarios vendedores verificados.</p><h3 className="text-orange-500 font-bold text-lg mt-4">3. Jurisdicción</h3><p>El uso de este sitio web y cualquier disputa que surja de su uso está sujeto a las leyes del comercio electrónico vigentes. TecnoByte LLC se reserva el derecho de entregar información de usuarios (KYC, IPs) requerida mediante órdenes judiciales en casos de fraude comprobado.</p></div></div>);
...
export default function App() {
  ...
  useEffect(() => {
     ...
     const unsub = onSnapshot(query(collection(db, 'artifacts', appId, 'public', 'data', 'orders'), where('seller.id', '==', user.uid)), (snap) => {
        ...
        snap.docChanges().forEach((change) => {
           ...
           if (change.type === "added") { playSound('notif', isMuted); setUnreadAlerts(u=>u+1); showNotification(`NUEVA VENTA RECIBIDA: #${d.orderId}`, "success"); }
           if (change.type === "modified" && d.status === 'payment_reported') { playSound('notif', isMuted); setUnreadAlerts(u=>u+1); showNotification(`PAGO REPORTADO EN #${d.orderId}`, "success"); }
        });
     });
     return () => unsub();
  }, [user, isMuted]);

  const showNotification = (msg, type = 'success') => { setNotification({ msg, type }); setTimeout(() => setNotification(null), 5000); };
  const handleLogout = async () => { playSound('click', isMuted); await signOut(auth); setUserData(null); setView('home'); showNotification("Sesión finalizada"); };
  ...
  if (loading) return (
    <div className="min-h-screen bg-black flex items-center justify-center text-yellow-500 font-gamer text-3xl relative overflow-hidden">
      <Styles /><div className="fixed inset-0 bg-black z-[-20]"></div><VideoBackground /><FireEffect />
      <div className="z-10 flex flex-col items-center p-10 hud-panel animate-pulse"><Crosshair size={120} className="text-orange-600 animate-spin-slow mb-6 drop-shadow-[0_0_20px_#FF4500]" /><span className="tracking-[0.2em] text-shadow-glow glitch" data-text="INICIANDO SISTEMA TECNOBYTE...">INICIANDO SISTEMA TECNOBYTE...</span></div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col relative text-gray-100 bg-transparent selection:bg-orange-600 selection:text-white">
      ...
      <VideoBackground /><LightningStorm /><FireEffect /><MeteorShower /><AshRain /><CharacterDecor />
